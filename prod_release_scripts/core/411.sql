...
(SELECT iri.jj_year,
       ...
       iri.sales_grp_nm,
       iri.scan_sales + 
       LAG(iri.scan_sales,1) OVER (PARTITION BY iri.iri_matl_id,iri.representative_cust_cd ORDER BY iri.iri_matl_id,iri.jj_mnth_id,iri.representative_cust_cd) + 
       LAG(iri.scan_sales,2) OVER (PARTITION BY iri.iri_matl_id,iri.representative_cust_cd ORDER BY iri.iri_matl_id,iri.jj_mnth_id,iri.representative_cust_cd) AS scan_sales
FROM (SELECT jj_year,
             jj_mnth_id,
             MAX(wk_end_dt) wk_end_dt,
             matl_id,
             coalesce (matl_id,iri_ean) as iri_matl_id,
             matl_desc,
             iri_ean,
             iri_market,
