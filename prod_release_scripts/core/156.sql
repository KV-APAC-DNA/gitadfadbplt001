create or replace view PROD_DNA_CORE.ASPEDW_ACCESS.EDW_VW_REGAINE_TW_INVOICE_RECORD_MASTER(
    "amount",
    "channel",
    "completed_date",
    "created_date",
...
left join prod_dna_core.aspitg_integration.itg_sfmc_consumer_master d 
on lower(c.subscriber_key) = lower(d.subscriber_key) 
where c.subscriber_key ilike 'JB_TW_%' and c.status = 'Approved' 
group by c.invoice_num;
create or replace view PROD_DNA_CORE.ASPEDW_ACCESS.EDW_VW_JB_TW_INVOICE_RECORD_DETAIL(
    "amount",
    "channel",
    "completed_date",
    "created_date",
