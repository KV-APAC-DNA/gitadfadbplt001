...
delete from PROD_DNA_CORE.ASPEDW_INTEGRATION.EDW_RPT_REGIONAL_SELLOUT_OFFTAKE WHERE country_code='VN' AND mnth_id>= (case when (select param_value from PROD_DNA_CORE.ASPITG_INTEGRATION.ITG_MDS_AP_CUSTOMER360_CONFIG where code='base_load_vn')='ALL' THEN '190001' ELSE to_char(add_months(to_date(convert_timezone('UTC',current_timestamp())), -((select param_value from PROD_DNA_CORE.ASPITG_INTEGRATION.ITG_MDS_AP_CUSTOMER360_CONFIG where code='base_load_vn')::integer)), 'YYYYMM') END);

insert into PROD_DNA_CORE.ASPEDW_INTEGRATION.EDW_RPT_REGIONAL_SELLOUT_OFFTAKE (

with wks_vietnam_regional_sellout_offtake_npd as 
(
	 select * from PROD_DNA_CORE.VNMWKS_INTEGRATION.WKS_VIETNAM_REGIONAL_SELLOUT_OFFTAKE_NPD
),
