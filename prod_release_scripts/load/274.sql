...
def main(session: snowpark.Session,Param):
    try:
        #Param=["2024-04-10_D.zip","THASDL_RAW.DEV_LOAD_STAGE_ADLS","dev/TH_POS_TESCO","SDL_TH_TESCO_TRANSDATA"] #just a test path 
        file_name1 = Param[0]
        stage_name = Param[1]
        ...
        with SnowflakeFile.open(full_path,''rb'',require_scoped_url=False) as fhandle:
            ...
            # Iterate over each file in the zip archive
                for file_name in zip_ref.namelist():
                    # Check if the file is a CSV file (you can adjust this condition as needed)
                    if file_name.endswith(''.xml''):
                        ...
                        with zip_ref.open(file_name) as file:
                            ...
                            df[''IR_DATE''] = pd.to_datetime(df[''IR_DATE''])
