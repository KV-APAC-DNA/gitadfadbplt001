CREATE OR REPLACE PROCEDURE JPDCLSDL_RAW.JPNDCL_SFMC_NOTSENT_PREPROCESSING("PARAM" ARRAY)
...
def main(session: snowpark.Session,Param):
    # Param=["extraction_table_ADD_20240719.csv","JPDCLSDL_RAW.DEV_LOAD_STAGE_ADLS_JPDCL","dev/DCL/jjrnki/extraction_table/","EXTRACTION_TABLE"]
    try:
        file_name       = Param[0]
        stage_name      = Param[1]
        temp_stage_path = Param[2]
...
def main(session: snowpark.Session,Param):
    ...
    try:
        ...
        df = df.with_column("source_file_date",lit(datetime.now(pytz.timezone("Asia/Singapore")).strftime("%Y%m%d")))
