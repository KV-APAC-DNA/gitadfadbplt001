...
create or replace stage ASPSDL_RAW.PROD_LOAD_STAGE_ADLS
 storage_integration = DEV_DNA_LOAD_AZURE3_SI
 url = 'azure://dlsadbplt001.blob.core.windows.net/asp/';
...
#''prd/transactional/CNSC'',''sdl_rg_travel_retail_cnsc_test2'']);';

CREATE OR REPLACE PROCEDURE ASPSDL_RAW.DFS_PREPROCESSING("PARAM" ARRAY)
RETURNS VARCHAR(16777216)
LANGUAGE PYTHON
...
def main(session: snowpark.Session,Param):
    #Param=["DFS_202009.csv","ASPSDL_RAW.PROD_LOAD_STAGE_ADLS","prd/transactional","sdl_rg_travel_retail_dfS"]
    try:
        # Extracting parameters from the input
        file_name = Param[0]
        stage_name = Param[1]
...
CREATE OR REPLACE PROCEDURE ASPSDL_RAW.DUFRY_PREPROCESSING("PARAM" ARRAY)
RETURNS VARCHAR(16777216)
LANGUAGE PYTHON
RUNTIME_VERSION = '3.8'
PACKAGES = ('snowflake-snowpark-python')
...
def main(session: snowpark.Session,Param): 
    ...
    try:
        ...
        #Param=[''Dufry_Hainan_122022.csv'',''PROD_DNA_LOAD.ASPSDL_RAW.PROD_LOAD_STAGE_ADLS'',''prd/transactional/Dufry'',''sdl_rg_travel_retail_dufry_hainan'']
...
def main(session: snowpark.Session,Param): 
    ...
    # Example input
    # Param=[''LSTR 112022.xlsx'',''last'',''1-1-1'',''LSTR'',''xlsx'',''Brand_name|Barcode|Item_Code|English_Desc|Chinese_Desc|Category|SRP_USD|Unit|Amt|Unit|Amt|Unit|Amt|Stock'',2,''ASPSDL_RAW.PROD_LOAD_STAGE_ADLS'',''prd/transactional/Lagardere'']
...
CREATE OR REPLACE PROCEDURE ASPSDL_RAW.HAINAN_PREPROCESSING("PARAM" ARRAY)
RETURNS VARCHAR(16777216)
LANGUAGE PYTHON
RUNTIME_VERSION = '3.11'
PACKAGES = ('snowflake-snowpark-python')
...
def main(session: snowpark.Session,Param): 
    # Your code goes here, inside the "main" handler.
    #Param=[''Hainan_Vendor_Sales_Report_for_Asian.csv'',''ASPSDL_RAW.PROD_LOAD_STAGE_ADLS'',''prd/transactional/dfs'',''sdl_rg_travel_retail_dfS_hainan'']
    try:
        # Extracting parameters from the input
        file_name       = Param[0]
        stage_name      = Param[1]
...
CREATE OR REPLACE PROCEDURE ASPSDL_RAW.LSTR_PREPROCESSING("PARAM" ARRAY)
RETURNS VARCHAR(16777216)
LANGUAGE PYTHON
RUNTIME_VERSION = '3.8'
PACKAGES = ('snowflake-snowpark-python')
...
CREATE OR REPLACE PROCEDURE ASPSDL_RAW.SHILLA_PREPROCESSING("PARAM" ARRAY)
RETURNS VARCHAR(16777216)
LANGUAGE PYTHON
RUNTIME_VERSION = '3.8'
PACKAGES = ('snowflake-snowpark-python')
