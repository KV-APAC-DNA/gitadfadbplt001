...
def main(session: snowpark.Session,Param):
    ...
    try:
        ...
        #Param=[''ROBINSONS_DS_20230521_20230601150011.xlsx'',''PHLSDL_RAW.DEV_LOAD_STAGE_ADLS'',''dev/non_pos/transaction/robinsons_ds/ph_non_pos_robinsons_ds/'',''sdl_ph_non_ise_robinsons_ds'']
...
def main(session: snowpark.Session,Param):
    ...
    try:
        ...
        #Param=[''ROBINSONS_SM_20210131_20210806154234.csv'',''PHLSDL_RAW.DEV_LOAD_STAGE_ADLS'',''dev/non_pos/transaction/robinsons_sm/ph_non_pos_robinsons_sm'',''sdl_ph_non_ise_svi_smc'']
        ...
        with SnowflakeFile.open(full_path,''rb'',require_scoped_url= False) as f:
            ...
            df32 = df2[7:]
            df31 = df2[:7]
            df32.dropna(subset=[''MONTH:''],inplace=True)
            df3 = pd.concat([df31,df32])
            verticle_df = df3[[''MONTH:'',''WEEK:'',''ACCOUNT: ROBINSONS SM'',''Actual Delivery Date:'',''OSA Check Date'',''Encoded Report'',''TEAM LEADER:'',''BRANCH CODE:'',''Branch Size and Classification'',''SKU CODE'']]

            dfh = df1[file_header_loc:]
            dfh.reset_index(inplace=True)
            dfh.columns = dfh.iloc[0]
            dfh = dfh[1:]
            
            valvars = []
            
            for i in dfh.columns[9:]:
                valvars.append(i)
            ...
            #Converting columns for branch name and osa flag
            unpivoted_df = dfh.melt(id_vars=idvars,value_vars=valvars,var_name=''branch_name'',value_name=''osa_flag'')
            ...
            joined_df = joined_df[joined_df[''SKU CODE''] != ''SKU:'']
            ...
            #snowparkdf = snowparkdf.filter((snowparkdf["branch_code"] != "DEL-W2, CRD-W1") & (snowparkdf["branch_code"] != "DEL-W2, CRD"))
