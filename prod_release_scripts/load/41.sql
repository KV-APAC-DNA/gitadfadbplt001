CREATE OR REPLACE PROCEDURE SGPSDL_RAW.SG_MARKETPLACE_PREPROCESSING("PARAM" ARRAY)
...
def main(session: snowpark.Session,Param):
    ...
    # Creating copy of the Dataframe
        ...
        and final_df.filter(col("CHANNEL").is_not_null()) and final_df.filter(col("MONTH").is_not_null())\\
        and final_df.filter(col("ORDER_CREATION_DATE").is_not_null()) and final_df.filter(col("STATUS").is_not_null())\\
        and final_df.filter(col("ITEM_CODE").is_not_null()) and final_df.filter(col("SALES_UNIT").is_not_null())\\
        and final_df.filter(col("NET_INVOICED_SALES").is_not_null()) and final_df.filter(col("BRAND").is_not_null())
